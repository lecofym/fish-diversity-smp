setwd("C:/Users/gramirezortiz/Documents/PNH_paper/PNH_github")
#install.packages("pacman")
library(pacman)
pacman:: p_load(lme4, jtools, vegan, arm, ggplot2,ade4,diveRsity, cluster, clue,FD,ggrepel,reshape2,dplyr,rcompanion,psych,car,mgcv,stats,tidyverse,emstreeR,ggConvexHull,Rcpp,geometry,rcdd,patchwork,plotly,tidyr,devtools,purrr,ape,gtools)
source("multidimFD.R")

####Calculation of ecological indicators####
#Read the density dataset
D_dataset<-read.csv("PNH_Density.csv",header=TRUE, row.names = 1)
names(D_dataset)
...
FM<-read.csv("PNH_Traits.csv", row.names=1)
...
a <- for (m in 1:nrow(weight.D)){
  FE[m,]<- dim(unique(FM[rownames(FM)%in% colnames(weight.D[m,apply(weight.D[m,],2,sum)>0]),]))
}
nbFE <- FE[,1]
sum(nbFE < 6)
...
pacman:: p_load(tidyverse, elbow)
source("quality_funct_space.R")
qfs <- quality_funct_space(FM, traits_weights = NULL,nbdim = 14, metric = "Gower", dendro = FALSE, plot = "NA")
round(qfs$meanSD , 4)
...
inflection_points<- data.frame('Dimensions' = 1:length(Var.expl),'Varianza explicada' = cumsum(Var.expl))
source("elbow.R")
perf <- elbow(data = inflection_points[, c('Dimensions', 'Varianza.explicada')])
...
write.csv(PCscores,file="PNH_PCoA_scores.csv")
...
check_the_names<-data.frame(rownames(D_dataset),rownames(weight1))
sum(check_the_names$rownames.D_dataset.==check_the_names$rownames.weight1)
...
NP.lmer.D <- lmer(log(Tot_weight,base=2) ~ Year_cons + (1|Site) + (1|Season),data= NP)
summ(NP.lmer.D)
...
NP.lmer.D.sca <- lmer(scale(log(Tot_weight,base=2)) ~ Year_cons + (1|Site)+ (1|Season),data= NP)
...
NP.fig.coef.model<- ggplot(coefs.data, aes(x= Indices, y=Estimate, fill = Indices)) + ggtitle("b) NP") + geom_hline(yintercept=0, lty=2, lwd=1, colour="black") + geom_errorbar(aes(ymin=Estimate - 1.96*se, ymax=Estimate + 1.96*se), lwd=1, colour= "black", width=0) + geom_point(size=20, pch=21, stroke = 1) + scale_fill_manual(values = rev(c("coral2","coral2","gray80", "gray80","gray80"))) + theme_bw() + theme(panel.grid = element_blank()) + theme(panel.background=element_rect(color="black",size=2),axis.text.x = element_text(size = 28, angle = 90), axis.title.x = element_text(size = 30, angle = 0),axis.text.y = element_blank(),axis.title.y = element_blank(),legend.position = "none") + theme(text=element_text(family="TT Arial")) + theme(plot.title = element_text(size = 30, face = "bold")) + coord_flip()
NP.fig.coef.model
...
MPA.lmer.D <- lmer(log(Tot_weight,base=2) ~ Year_cons + (1|Site) + (1|Season),data= MPA)
summ(MPA.lmer.D)
...
MPA.lmer.D.sca <- lmer(scale(log(Tot_weight,base=2)) ~ Year_cons + (1|Site) + (1|Season),data= MPA)
...
MPA.fig.coef.model<- ggplot(MPA.coefs.data, aes(x= Indices, y=Estimate, fill = Indices)) + ggtitle("a) MPA") + geom_hline(yintercept=0, lty=2, lwd=1, colour="black") + geom_errorbar(aes(ymin=Estimate - 1.96*se, ymax=Estimate + 1.96*se), lwd=1, colour= "black", width=0) + geom_point(size=20, pch=21, stroke = 1) + scale_fill_manual(values = rev(c("dodgerblue3", "dodgerblue3","dodgerblue3","dodgerblue3","dodgerblue3"))) + theme_bw() + theme(panel.grid = element_blank()) + theme(panel.background=element_rect(color="black",size=2),axis.text.x = element_text(size = 28, angle = 90), axis.title.x = element_text(size = 30, angle = 0),axis.text.y = element_text(size = 28, angle = 0),axis.title.y = element_blank(),legend.position = "none") + scale_y_continuous (breaks= seq(-0.1, 0.5, 0.05)) + theme(text=element_text(family="TT Arial")) + theme(plot.title = element_text(size = 30, face = "bold")) + coord_flip()
MPA.fig.coef.model

Estimate.graph <- MPA.fig.coef.model + NP.fig.coef.model + plot_layout(nrow=1, guides="collect")

ggsave('Figure_3.tiff', plot = Estimate.graph,
       width = 4500, height = 5700, units = 'px', dpi = 320)

####Spatial analyses of fish diversity####
#As trait matrix we use PCoA scores calculated in the previous section
coord <- read.csv("PNH_PCoA_scores.csv")
names(coord)
coord <-data.frame(coord[c(1:6)]);colnames(coord)<-c("name","E1","E2","E3","E4", "E5")
#As weight matrix we use the the average abundance per species at each human use level
assemblage.use <- read.csv("PNH_spatial_analysis.csv")
coord_com<-assemblage.use %>% pivot_longer(cols=!Human_use_level) %>% full_join(coord,by="name") %>% filter(value > 0) %>% mutate_at("Human_use_level", factor, levels=c("MPA","Non-protected")) %>% group_by(Human_use_level) %>% mutate(centroid.E1 = mean(E1),centroid.E2 = mean(E2),centroid.E3 = mean(E3),centroid.E4 = mean(E4),centroid.E5 = mean(E5)) %>% group_by(Human_use_level,name) %>%mutate(dist.cen = sqrt(sum((c(E1,E2,E3,E4,E5) - c(centroid.E1,centroid.E2,centroid.E3,centroid.E4,centroid.E5)) ^ 2))) %>% ungroup()

#Abundance weighted centroid
trait_centroid<- coord_com %>% group_by(Human_use_level) %>% mutate(value = value/sum(value)) %>% mutate(E1 = E1 * value) %>% mutate(E2 = E2 * value) %>% mutate(E3 = E3 * value) %>% mutate(E4 = E4 * value) %>% mutate(E5 = E5 * value) %>% summarise(across(E1:E5, ~mean(.x)))
...
(F_div1 <- coord_com %>%
    ...
    mutate(dist.cen = sqrt(sum((c(E1,E2,E3,E4,E5) - c(centroid.E1,
                                                   centroid.E2,
                                                   centroid.E3,
                                                   centroid.E4,centroid.E5)) ^ 2))) %>%
    ggplot(aes(x=E1,y=E2))+
    geom_point(aes(size=value, colour=Human_use_level), alpha=.3) +
    geom_point(data=trait_centroid, aes(x=E1,y=E2,fill=Human_use_level),
...
(F_div2 <- coord_com %>%
    ...
    geom_point(aes(size=value, colour=Human_use_level), alpha=.3) +
    geom_point(data=trait_centroid, aes(x=E3,y=E4,fill=Human_use_level),
               shape=23, colour=cpalette,size=6)+
    stat_ellipse(data = subset(coord_com, coord_com$Human_use_level == "MPA"),
...
coord <- read.csv("PNH_PCoA_scores.csv", row.names = 1)
names (coord)
trait2 <- data.frame(coord[c(1:5)])
colnames(trait2)<-c("E1","E2","E3","E4","E5")
assemblage.use <- read.csv("PNH_spatial_analysis.csv", row.names = 1)
...
#```{r}
(F_ori1 <- coord_com %>%
    mutate(overall.cen.E1 = mean(E1),
           overall.cen.E2 = mean(E2)) %>%
    ggplot(aes(x=E1,y=E2, colour=Human_use_level))+
    geom_point(aes(size=value), alpha=.7)+
    # ggrepel::geom_text_repel(aes(label=name))+
    geom_segment(data= pair_data_ori, aes(xend=E1,x=E1_end,yend=E2,y=E2_end, colour=Human_use_level), size=.2)+
    scale_colour_manual(values=cpalette)+
    scale_fill_manual(values=cpalette)+
    scale_size(range=c(0,7))+
    labs(x="PCoA1", y="PCoA2", colour = "Level", size= "Density",title="c) FOri")+
...
#```{r}
(F_ori2 <- coord_com %>%
    mutate(overall.cen.E3 = mean(E3),
           overall.cen.E4 = mean(E4)) %>%
    ggplot(aes(x=E3,y=E4, colour=Human_use_level))+
    geom_point(aes(size=value), alpha=.7)+
    # ggrepel::geom_text_repel(aes(label=name))+
    geom_segment(data= pair_data_ori2, aes(xend=E3,x=E3_end,yend=E4,y=E4_end, colour=Human_use_level), size=.2)+
    scale_colour_manual(values=cpalette)+
    scale_fill_manual(values=cpalette)+
    scale_size(range=c(0,7))+
...
ggsave('Figure_2.tiff', plot = Spatial.graph,
       width = 5700, height = 5700, units = 'px', dpi = 320)
